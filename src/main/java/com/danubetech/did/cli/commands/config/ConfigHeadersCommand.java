...
@CommandLine.Command(
        name = "headers",
        description = "Get or set the display of the API headers for display purposes. Default value: " + Headers.DEFAULT_HEADERS + ".",
...
public class ConfigHeadersCommand extends ConfigAbstractCommand implements Callable<Integer> {
    ...
    @CommandLine.Parameters(
            index = "0",
            description = "The display of the API headers. Valid values: ${COMPLETION-CANDIDATES}. Default value: " + Headers.DEFAULT_HEADERS + ".",
            arity = "0..1"
    )
